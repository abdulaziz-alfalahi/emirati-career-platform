...
const TypeAssertionSupabaseService = {
  ...
  linkedin: {
    ...
    getProfileByUsername: async (username: string): Promise<APIResponse<any>> => {
      return await LinkedInService.getProfileByUsername(username);
    },
    ...
    searchPeople: async (
      ...
      return await LinkedInService.searchPeople(keywords, options);
    },
    
    // Import LinkedIn profile to resume
    importProfileToResume: async (profile: any): Promise<APIResponse<any>> => {
      try {
        const resumeData = await LinkedInService.importProfileToResume(profile);
